...
class DatabaseFactory
{
    ...
    if (!$this->database) {
        ...
        /**
         * Check DB connection in try/catch block. Also when mysqli is not constructed properly,
         * prevent exposing database host, username and password in plain text
         * by throwing a custom error message
         */
        try {

            // @ unterdrückt technische Fehlermeldungen, damit nichts Sensibles angezeigt wird
            $mysqli = @new mysqli(
                Config::get('DB_HOST'),
                Config::get('DB_USER'),
                Config::get('DB_PASS'),
